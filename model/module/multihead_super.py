...
class AttentionSuper(nn.Module):
    def __init__(self, super_embed_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., normalization = False, relative_position = False,
                 num_patches = None, max_relative_position=14, rank_ratio=0.9, scale=False, change_qkv = False):
        super().__init__()
        ...
        qkv_rank = int(super_embed_dim * self.rank_ratio)
        if change_qkv:
            self.qkv1 = qkv_super(super_embed_dim, qkv_rank, bias=False)
            self.qkv2 = qkv_super(qkv_rank, qkv_rank, bias=False)
            self.qkv3 = qkv_super(qkv_rank, 3 * super_embed_dim, bias=qkv_bias)

        else:
            self.qkv1 = LinearSuper(super_embed_dim, qkv_rank, bias=False)
            self.qkv2 = LinearSuper(qkv_rank, qkv_rank, bias=False)
            self.qkv3 = LinearSuper(qkv_rank, 3 * super_embed_dim, bias=qkv_bias)
        ...
        proj_rank = int(super_embed_dim * self.rank_ratio)
        self.proj1 = LinearSuper(super_embed_dim, proj_rank, bias=False)
        self.proj2 = LinearSuper(proj_rank, proj_rank, bias=False)
        self.proj3 = LinearSuper(proj_rank, super_embed_dim)
    ...
    def set_sample_config(self, sample_q_embed_dim=None, sample_num_heads=None, sample_in_embed_dim=None):
        ...
        qkv_rank = int(self.super_embed_dim * self.rank_ratio)
        self.qkv1.set_sample_config(sample_in_dim=sample_in_embed_dim, sample_out_dim=qkv_rank)
        self.qkv2.set_sample_config(sample_in_dim=qkv_rank, sample_out_dim=qkv_rank)
        self.qkv3.set_sample_config(sample_in_dim=qkv_rank, sample_out_dim=3*self.sample_qk_embed_dim)        

        proj_rank = int(self.super_embed_dim * self.rank_ratio)
        self.proj1.set_sample_config(sample_in_dim=self.sample_qk_embed_dim, sample_out_dim=proj_rank)
        self.proj2.set_sample_config(sample_in_dim=proj_rank, sample_out_dim=proj_rank)
        self.proj3.set_sample_config(sample_in_dim=proj_rank, sample_out_dim=sample_in_embed_dim)
    ...
    def get_complexity(self, sequence_length):
        total_flops = 0
        total_flops += self.qkv1.get_complexity(sequence_length)
        total_flops += self.qkv2.get_complexity(sequence_length)
        total_flops += self.qkv3.get_complexity(sequence_length)
        # attn
        total_flops += sequence_length * sequence_length * self.sample_qk_embed_dim
        # x
        total_flops += sequence_length * sequence_length * self.sample_qk_embed_dim
        total_flops += self.proj1.get_complexity(sequence_length)
        total_flops += self.proj2.get_complexity(sequence_length)
        total_flops += self.proj3.get_complexity(sequence_length)
        if self.relative_position:
    ...
    def forward(self, x):
        ...
        x = self.qkv1(x)
        x = self.qkv2(x)
        qkv = self.qkv3(x).reshape(B, N, 3, self.sample_num_heads, -1).permute(2, 0, 3, 1, 4)
